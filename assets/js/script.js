...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const sections = gsap.utils.toArray(".cleaning-animate-wrap");

    sections.forEach((section) => {
        const cards = section.querySelectorAll(".cleaning-animate-card");
...
//   Cleaning service banner stagger JS
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    tl.fromTo(
        [".cleaning-services-banner .hero-img", ".cleaning-services-banner h1"],
        { opacity: 0, y: 40 },
        { opacity: 1, y: 0, duration: 1, stagger: 0 }
    )
        .fromTo(
            ".cleaning-services-banner p",
            { opacity: 0, y: 30 },
            { opacity: 1, y: 0, duration: 0.8 },
            "-=0.6"
        )
        .fromTo(
            ".cleaning-services-banner .mega-reveal-btn",
            { opacity: 0, y: 25, scale: 0.95 },
            { opacity: 1, y: 0, scale: 1, duration: 0.6 },
            "-=0.5"
        )
        .fromTo(
            ".cleaning-services-banner ul li",
            { opacity: 0, x: -20 },
            { opacity: 1, x: 0, duration: 0.5, stagger: 0.12 },
            "-=0.4"
        )
        .fromTo(
            ".cleaning-services-banner .border-t",
            { opacity: 0, y: 20 },
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".cleaning-heading-reveal").forEach((heading) => {
        gsap.fromTo(
            heading,
            {
                opacity: 0,
...
// Cleaning service marquee JS
document.addEventListener('DOMContentLoaded', function () {
    new Splide('.cleaning-marque', {
        type: 'loop',
        drag: 'free',
        focus: 'center',
        perPage: 7,
...
// Cleaning testimonial slider
document.addEventListener("DOMContentLoaded", () => {
  const slider = document.querySelector("#cleaning-testimonial-slider");
  ...
  // Check if we are on a mobile/tablet device (less than 1024px)
  ...
  const splide = new Splide(slider, {
    ...
    drag: "free",
    focus: "center",
    perPage: 3, // Default for 1440px and above
    gap: "32px",
    arrows: false,
    pagination: true,
    
    // Configure AutoScroll options, disabling it below 1024px
    autoScroll: isMobileOrTablet ? false : {
      speed: 1,
      pauseOnHover: true,
      pauseOnFocus: true,
    },
    ...
    breakpoints: {
      1439: { // 2 items between 1024px and 1439px
        perPage: 2,
        gap: "24px",
      },
      980: { // 1 item below 1024px
        perPage: 1,
        gap: "16px",
        drag: true, 
      },
  ...
  // Mount Splide with the AutoScroll extension if available and screen is >= 1024px
  const extensions = window.splide?.Extensions || {};
  if (extensions.AutoScroll && !isMobileOrTablet) {
    splide.mount({ AutoScroll: extensions.AutoScroll });
  } else {
    splide.mount();
  }
});
...
// 
document.addEventListener("DOMContentLoaded", function () {
    const faqItems = document.querySelectorAll(".cleaning-faq-item");

    faqItems.forEach(function (item) {
        const trigger = item.querySelector(".cleaning-faq-trigger");
        const content = item.querySelector(".cleaning-faq-content");
        const icon = item.querySelector(".cleaning-icon-close img");
        ...
        trigger.addEventListener("click", function () {
            ...
            faqItems.forEach(function (faq) {
                ...
                const faqContent = faq.querySelector(".cleaning-faq-content");
                const faqIcon = faq.querySelector(".cleaning-icon-close img");
